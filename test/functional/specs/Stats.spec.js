    ...
    request(sails.hooks.http.app)
      ...
      .end(function(err, response) {
        ...
        for (let x in WELL_ANSWER['sites']) {
          response.body['sites'].should.contain.a.thing.with.property('location_type', WELL_ANSWER['sites'][x].location_type)
          response.body['sites'].should.contain.a.thing.with.property('count', WELL_ANSWER['sites'][x].count)
        }

        // Testing paths
        for (let x in WELL_ANSWER['paths']) {
          response.body['paths'].should.contain.a.thing.with.property('type', WELL_ANSWER['paths'][x].type)
          response.body['paths'].should.contain.a.thing.with.property('distance', WELL_ANSWER['paths'][x].distance)
        }
        done()
      })
  })
