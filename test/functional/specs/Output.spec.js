  ...
  it('should export a feature array with equal size (project)', function(done) {
    request(sails.hooks.http.app)
      ...
      .end(function(err, response) {
        ...
        chai.assert.strictEqual(well2content.features.length, response.body.features.length)
        done()
      })
  })
    ...
    request(sails.hooks.http.app)
      ...
      .end(function(err, response) {
        ...
        for (let x in well2content.features) {
          ...
          let flag = false
          for (let y = 0; y < response.body.features.length && !flag; ++y) {
            let content2 = response.body.features[y].geometry
            if (content.type === content2.type && content.type === 'Point') {
              flag = (content2.coordinates[0] === content.coordinates[0]) && (content2.coordinates[1] === content.coordinates[1])
            } else if (content.type === content2.type && content.type === 'LineString') {
              for (let z = 0; z < content.coordinates.length && !flag; ++z) { // In this case the flag indicates there is a fail (we invalidate)
                if (content2.coordinates[z] && content.coordinates[z]) {
                  flag = !((content2.coordinates[z][0] === content.coordinates[z][0]) && (content2.coordinates[z][1] === content.coordinates[z][1]))
                }
              }
