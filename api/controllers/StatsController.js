...
module.exports = {
  project: async function (req, res) {
    ...
    if (!project) res.badRequest({msg: 'There is no project with given id.'});
    ...
    let PATH_STATS_SQL = `SELECT p.${Path.schema.type.columnName}, sum(p.${Path.schema.distance.columnName}) as 'distance'
    ...
    let CABLE_STATS_SQL = `SELECT DISTINCT ${Path.schema.type.columnName}, sum(${Path.schema.distance.columnName}) as 'distance'
      FROM (SELECT DISTINCT p.${Path.schema.type.columnName}, p.${Path.schema.distance.columnName}
        FROM ${Path.tableName} p, cable_intermedial__path_cables cp, ${Cable.tableName} c
        WHERE p.${Path.schema.project.columnName} = $1 AND p.${Path.schema.id.columnName} = cp.path_cables AND c.${Cable.schema.id.columnName} = cp.cable_intermedial) as unic
      ...
      FROM (SELECT DISTINCT p.${Path.schema.distance.columnName}
		...
		try {
			locationResult = await datastore.sendNativeQuery(LOCATION_STATS_SQL, [project.id]);
      pathResult = await datastore.sendNativeQuery(PATH_STATS_SQL, [project.id]);
      cableResult = await datastore.sendNativeQuery(CABLE_STATS_SQL, [project.id]);
		} catch (err) {
			res.serverError(err);
		}
