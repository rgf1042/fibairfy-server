...
async function exportData(criteriaSites, criteriaPaths) {
    ...
    let paths;
    try {
        paths = await Path.find(criteriaPaths)
            .populate('first')
            .populate('last');
    } catch (err) {
        throw err;
        ...
        try {
            ...
            for (let x in positions) {
                ...
                if (positions[x].geometry.type === 'Point') {
                    ...
                    if (positions[x].properties) {
                        // We have to test if it's possible somewhere
                        if (positions[x].properties.name)
                            site['name'] =
                                positions[x].properties.name ||
                                'site-' + uuidv1();
                        if (positions[x].properties.type)
                            site['type'] = positions[x].properties.type;
                        if (positions[x].properties.status)
                            site['status'] = positions[x].properties.status;
                    }
                    ...
                    for (let y in positions[x].geometry.coordinates) {
                        ...
                        path.intermedial.push([latitude, longitude]);
                    }
                    if (flag) {
                        paths.push(path);
                    }
            ...
            for (let x in paths) {
                ...
                if (posIni < 0) {
                    // In this case we have to create a Site
                    let newSite = {};
            ...
            Promise.all(sitesPromises)
                .then(async function(values) {
                    for (let x in paths) {
                        ...
                        let distance;
                        try {
                            distance = await sails.helpers.getDistancePath(
                                values[path.posIni].id,
                                values[path.posEnd].id,
                                path.intermedial
                            );
                        } catch (err) {
                            reject(err);
        ...
        req.file('data').upload(
            ...
            function whenDone(err, uploadedFiles) {
                ...
                if (mime.lookup(uploadedFiles[0].fd) === mime.lookup('.kmz')) {
                    ...
                    mime.lookup(uploadedFiles[0].fd) === mime.lookup('.geojson')
                ) {
                    ...
                    mime.lookup(uploadedFiles[0].fd) === mime.lookup('.kml')
                ) {
                    ...
                    return res.json({
                        msg: 'No file was uploaded or invalid file format.',
                    });
                }
            }
        );
