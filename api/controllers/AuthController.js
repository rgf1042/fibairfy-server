    ...
    login: function(req, res) {
        ...
        User.findOne({ username: username }, function(message, user) {
            ...
            if (user.password === '' || user.password === undefined)
                return res.json({
                    flag: false,
                    message: 'This user doesnt have password. Try with LDAP.',
                });
    ...
    loginLDAP: function(req, res) {
        ...
        ldap.search(username, function(data) {
            if (data.error)
                return res
                    .status(401)
                    .json({ flag: false, message: data.error });
            if (data.cn === 'lectura') {
                ldap.authenticate(
                    ...
                    function(auth) {
                        if (auth.error) {
                            ...
                            User.findOne({ username: username }, function(
                                ...
                                if (!user) {
                                    // User is not in fiberfy database we create one
                                    User.create(
                                        { username: username, isLdap: true },
                                        function(err, user) {
                                            if (err)
                                                res.serverError({
                                                    flag: false,
                                    ...
                                    // There's this user inside fiberfy database
                                    if (!user.isLdap) {
                                        // Exists but it's not LDAP
                                        User.update(
                                            {
                                                username: username,
                                                isLdap: true,
                ...
                // User it's not in LDAP or it doesn't have enough permissions
                return res.json({
                    flag: false,
                    message: 'invalid username or password',
                });
