...
module.exports = {
        ...
        async.reduce(
            ...
            function(memo, attrName, nextAttrName) {
                ...
                if (
                    ...
                    req._sails.models[attrDef.collection].attributes[
                        attrDef.via
                    ].model
                ) {
            ...
            async function(err, removedFromNotificationsToSend) {
                ...
                try {
                    data.distance = await sails.helpers.getDistancePath(
                        data.first,
                        data.last,
                        data.intermedial
                    );
                        ...
                        Model.findOne(
                            ...
                            if (!populatedRecord) {
                                return res.serverError(
                                    'Could not find record after creating!'
                                );
                            }
                            ...
                            if (req._sails.hooks.pubsub) {
                                if (req.isSocket) {
                                    Model.subscribe(req, [
                                        populatedRecord[Model.primaryKey],
                                    ]);
                                ...
                                if (removedFromNotificationsToSend.length) {
                                    _.each(
                                        removedFromNotificationsToSend,
    ...
    update: function(req, res) {
        ...
        query.exec(async function found(err, matchingRecord) {
            ...
            if (
                queryOptions.valuesToSet.first ||
                queryOptions.valuesToSet.last ||
                queryOptions.valuesToSet.intermedial
            ) {
                let first =
                    queryOptions.valuesToSet.first || matchingRecord.first;
                let last = queryOptions.valuesToSet.last || matchingRecord.last;
                let intermedial =
                    queryOptions.valuesToSet.intermedial ||
                    matchingRecord.intermedial;

                try {
                    queryOptions.valuesToSet.distance = await sails.helpers.getDistancePath(
                        first,
                        last,
                        intermedial
                    );
